# 품앗이웃 — 생활지원 연결 에이전트 데모

자연어 요청을 별도 안전 게이트로 먼저 판정한 뒤 최대 3개의 작업으로 분리하고, 매일 반복되는 가능 시간대와
...
추천 계산과 최종 위험도 상향 규칙은 모델이 아닌 Python 코드에서 수행합니다.
Safety Triage Agent가 입력을 먼저 분류하고, 승인된 내용만 작업 에이전트가 1~3개로
분리합니다. UI에서 선택한 날짜와 시간이 자연어보다 우선합니다.
...
- `low`: 생활·정서지원 작업으로 분해하고 바로 후보 검색
- `mid`: 작업으로 분해하되 `그래도 도움 요청` 확인 전에는 후보를 검색하지 않음
- `high`: 해당 요청 구간만 폐기하고 나머지 안전한 요청은 계속 처리
- `emergency`: 전체 입력을 차단하고 `119 전화 연결` 표시
